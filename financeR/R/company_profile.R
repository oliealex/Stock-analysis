...
company_profile <- function(tickers, api_key = api){
  if(length(tickers) > 1){
    ...
    for (i in 1:length(tickers)) {
      res <- GET(paste0("https://financialmodelingprep.com/api/v3/profile/",tickers[i],"?apikey=",api_key))
      data[i,1:32] <- fromJSON(rawToChar(res$content))
    }
  }
  else{
    res <- GET(paste0("https://financialmodelingprep.com/api/v3/profile/",tickers,"?apikey=",api_key))
    data <- fromJSON(rawToChar(res$content))
  }
  return(data)
}
